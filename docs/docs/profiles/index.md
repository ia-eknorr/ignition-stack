...
The profile you pick is saved in the stack's [configuration record](../concepts/configuration-record.md). `ignition-stack switch-profile <slug>` then reshapes the same stack under a different profile, carrying your database, services, reverse-proxy, and edge choices across. See the [reset and reshape guide](../guides/reset-and-reshape.md).

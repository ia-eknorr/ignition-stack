...
``switch-profile`` need: they read it back, optionally reshape it, and re-run
...
``reset`` / ``switch-profile`` can fail cleanly on a directory that was never
...
def read_record(project_dir: Path) -> ProjectConfig:
    ...
    if not src.is_file():
        raise LifecycleError(
            ...
            "ignition-stack (or its record was removed); reset/switch-profile "
